  ...
  function of(text, opts) {
    ...
    function meanings(lemmas) {
      ...
      lemmas.forEach(function (l) {
        var x = gloss[l];
        String(x && x[1] || "").split(/\s*\/\s*/).forEach(function (p) { if (p && parts.indexOf(p) < 0) parts.push(p); });
      });
      return parts.join(" / ");
    }
    ...
    function guess(w) {
      if (hitsOf(w).length || (gloss[w] && gloss[w][0] !== w) || nominal(w) || isInfinitive(w)) return;
    ...
    function cliticFn(c, v, i) {
      ...
      if (has(fns, "refl") && samePerson) {
        ...
        if (!reflLemma && mk && l0 && !isInfinitive(v.w)) made = l0.replace(new RegExp(mk[0]), mk[1]);
    ...
    function cliticAt(i) {
      ...
      return { kind: "clitic", w: t.w, base: key, es: (c.esFn && c.esFn[f.fn]) || c.es, fn: f.fn, refl: f.lemma || "", verb: T[v.i].w };
    }
    ...
    function compoundAt(i) {
      ...
      if (re && hasClitic && lemmas.some(function (l) { return re.test(l); }) && T[i - 1].w !== "l'") {
        lemmas = lemmas.filter(function (l) { return re.test(l); }).concat(lemmas.filter(function (l) { return !re.test(l); }));
      }
      var tenses = [], persons = [];
    ...
    function encliticAt(i) {
      ...
      return { kind: "encl", w: w, base: r.shown, suf: suf, sufEs: (E.es || {})[suf] || suf, lemma: r.lemma, form: r.form, rloss: !!r.rloss,
    ...
    function baseVerb(base, suf) {
      ...
      if (!E.hyphen && /r$/.test(base)) cands.push({ inf: base + "e", form: "infinitivo", shown: base + "e" });
    ...
    function moodFilter(hs, i) {
      ...
      for (var k = Math.max(0, i - 6); k < i; k++) if (has(D.subjTriggers, T[k].w) && !T[k].clause) trig = true;
    ...
    for (var i = 0; i < T.length; i++) {
      ...
      var afterSubj = !!SUBJ[prev] || !!wasClitic[i - 1];
      ...
      var hard = i > 0 && !!arts[prev];                 // article, contraction, preposition
      ...
      if (hits.length && noun) {
        var asVerb = afterSubj || !!(D.idioms && D.idioms[w]) ||
          (!soft && has(D.interrog, prev) && !(next && nominal(next)) && !verbAt(i + 1));
        if (!asVerb) {
          var alt = !soft && !t.clause && !has(D.notVerb, w) && hits.some(finite) ? verbEntry(w, hits.filter(finite), null, i) : null;
    ...
    return es.map(function (e) {
      ...
      if (e.kind === "clitic") {
        ...
        return who + ": pronombre átono «" + e.es + "», " + fnText(e.fn) +
          (e.refl ? " (*" + e.refl + "*)" : "") + ", delante de *" + e.verb + "*" +
      ...
      if (e.kind === "encl") {
        var how = e.form === "infinitivo" ? (e.rloss ? "infinitivo: delante de *-lo*, *-la* pierde la *-r* y lleva tilde" :
          ...
          "de *" + e.lemma + "*, " + tenseText(e.tenses, e.persons, week);
